...
#define G 9.86           // Gravitational constant
...
double InputFunction(double M1_r,double M2_r, double L1_r,double L2_r,double IC1,double IC2);
void DefineValues(double M1_r, double M2_r, double L1_r, double L2_r, double IC1, double IC2);
...
int main(int argc, char *argv[]) { 
	...
	//Multiplies the provided mulitiple of pi with pi for intitial angle
	IC1 = IC1 * M_PI;
	IC2 = IC2 * M_PI;
    
    // Calculate masses and lengths based on the ratios
    double M1 = M1_ratio; // Mass of pendulum 1 (arbitrary units)
    double M2 = M2_ratio; // Mass of pendulum 2 (arbitrary units)
    double L1 = L1_ratio * 100.0; // Length of pendulum 1 (pixels)
    double L2 = L2_ratio * 100.0; // Length of pendulum 2 (pixels)
...
double InputFunction(double M1_r,double M2_r, double L1_r,double L2_r,double IC1,double IC2){
	...
	printf("For ratios (x:y), Enter vales as: x y \n");
	//Prompts and accets input from User for mass/length ratios, and Initial angles
    printf("Enter mass ratio (M1:M2) e.g., 1 2 for 1:2: ");
    scanf("%lf %lf", &M1_ratio, &M2_ratio);
    
	//While loop to prompt the user to choose a valid number that will fit in the display
	while(1){
    printf("Enter length ratio (L1:L2) whose difference is not greater than 4: ");
    scanf("%lf %lf", &L1_ratio, &L2_ratio);
	double absolutevalue = fabs(L1_ratio - L2_ratio);
	if(absolutevalue > 4){
	...
	return M1_r, M2_r, L1_r, L2_r, IC1, IC2;
}
